...
async function crearCliente(req, res) {
  try {
    ...
    if (yaExiste) return res.status(409).json({ error: 'Ya existe un cliente con ese correo' });
...
async function listarClientes(req, res) {
  try {
    ...
    const { rows, count } = await Cliente.findAndCountAll({ where, limit: Number(limit), offset, order: [['fecha_registro', 'DESC']] });
    return res.json({ total: count, pagina: Number(page), totalPaginas: Math.ceil(count / Number(limit)), clientes: rows });
  } catch (err) {
...
async function obtenerCliente(req, res) {
  try {
    const cliente = await Cliente.findByPk(req.params.id, { include: [{ model: Interaccion, as: 'interacciones' }] });
...
async function actualizarEtapa(req, res) {
  try {
    ...
    if (!etapasValidas.includes(etapa_crm)) return res.status(400).json({ error: `etapa_crm debe ser una de: ${etapasValidas.join(', ')}` });
    const cliente = await Cliente.findByPk(req.params.id);
    if (!cliente) return res.status(404).json({ error: 'Cliente no encontrado' });
...
module.exports = { crearCliente, listarClientes, obtenerCliente, actualizarCliente, eliminarCliente, actualizarEtapa };
