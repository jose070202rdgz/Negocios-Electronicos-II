...
async function crearInteraccion(req, res) {
  try {
    ...
    if (!cliente_id || !tipo || !descripcion) {
      return res.status(400).json({ error: 'cliente_id, tipo y descripcion son obligatorios' });
    }

    if (!tiposValidos.includes(tipo)) {
      return res.status(400).json({ error: `tipo debe ser una de: ${tiposValidos.join(', ')}` });
    }
...
async function listarTodasLasInteracciones(req, res) {
  try {
    const interacciones = await Interaccion.findAll({
      include: [
